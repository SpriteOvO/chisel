  ...
  object util {
    ...
    def convert(ports: Seq[Port], topDir: SpecifiedDirection = SpecifiedDirection.Unspecified): Ports = {
      ...
      Ports(
        ...
        symAttrs = Seq.empty, // irs.map(port => circt.hwInnerSymAttrGet(port.name)),
    ...
    def emitConnect(dest: Reference.Value, srcVal: Reference.Value, loc: MlirLocation): Unit = {
      ...
      if (dest.tpe.isInstanceOf[fir.VectorType]) {
        // Connect all the vector elements pairwise.
        val numElements = dest.tpe.asInstanceOf[fir.VectorType].size
        assert(src.tpe.isInstanceOf[fir.VectorType])
        assert(src.tpe.asInstanceOf[fir.VectorType].size == numElements)
